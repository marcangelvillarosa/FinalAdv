...
import { collection, setDoc, doc, serverTimestamp, orderBy, query, onSnapshot, where, getDocs, updateDoc, arrayRemove, arrayUnion} from "firebase/firestore";
...
const FriendProfile = () => 
{
    ...
    const handleUpload = (e) => {
      ...
      useEffect(() => {
        const getUserProfile = async () => {
                ...
                const userPost = query(collection(db, "posts"), where("uid", "==", id));
                const postData = await getDocs(userPost);
    ...
    const [userDatass, setUserDatas] = useState([]);
    const [friendCounts, setFriendCount] = useState(0);

    useEffect(() => {
      const fetchUserProfile = async () => {
          try {
              // Query the users collection to get the user's profile
              const userQuery = query(collection(db, "users"), where("uid", "==", friendId));
              const userDataSnapshot = await getDocs(userQuery);
              const userData = userDataSnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }));

              if (userData.length > 0) {
                  setProfile(userData);
                  const userFriends = userData[0]?.friends || []; // Assuming the friends are stored in a field named 'friends'
                  setFriendCount(userFriends.length); // Set the count of friends
                  // Fetch the profiles of the user's friends
                  const friendsPromises = userFriends.map(async (friend) => {
                      const friendQuery = query(collection(db, "users"), where("uid", "==", friend.id));
                      const friendDataSnapshot = await getDocs(friendQuery);
                      return friendDataSnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
                  });
                  const friendsData = await Promise.all(friendsPromises);
                  setUserDatas(friendsData.flat()); // Flatten the array of arrays and set the state
              }
          } catch (error) {
  ...
  // Filtered array of friends based on search term
  const filteredFriends = userDatass.filter(friend => {
    // Convert both friend name and search term to lowercase for case-insensitive search
    const friendName = friend.name.toLowerCase();
    const searchTermLowerCase = input.toLowerCase();
    // Return true if the friend's name contains the search term
    return friendName.includes(searchTermLowerCase);
});
    ...
    return (
        <div className="w-screen h-screen bg-gray-200 overflow-y-scroll overflow-x-hidden">
            ...
            <div className="w-[100%] h-[70%] bg-white flex items-center justify-center flex-col">
                
                <div className="w-[68.4%] h-[65%] bg-gray-300 rounded-bl-xl rounded-br-xl">
                ...
                <div className="w-[68.4%] h-[35%] flex items-center justify-center flex-col">
                    ...
                    <div className="w-[100%] h-[70%] flex items-center justify-center">
                        ...
                        <div className="w-[15%] h-[100%]">
                            ...
                            {profile.map((profile) =>{ 
                                 return  <Avatar  variant="circular" className="w-[100%] h-[100%] object-cover" src={profile?.image || require('../pics/nickname.png')} alt="avatar"></Avatar>       
                                 })}
                            </div>
                        </div>
                        ...
                        <div className="w-[40%] h-[100%]">
                            ...
                            <div className="w-[100%] h-[43%] flex items-end pb-2">
                                {profile.map((profile) =>{ return <div>
                                    <h1 className="font-semibold text-3xl">{profile.name}</h1>
                                </div>})}
                            </div>
                           
                            <div className="w-[100%] h-[20%] flex items-start">
                               <h1 className="font-semibold text-gray-800">{friendCounts} friends</h1>
                            </div>
                            
                            <div className="w-[100%] h-[37%] flex flex-row">
                              {userDatass.map((friend, index) => (
                                    <div key={index}>
                                          <Avatar size="sm" variant="circular" src={friend.image || require('../pics/nickname.png')} alt={friend.name} />
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="w-[40%] h-[100%]">
                          <div className="w-[100%] h-[50%]">
                            ...
                            <div className="w-[100%] h-[50%] flex justify-end items-center relative">
                                <div className="w-[26%] h-[42%] m-1">
                                    <Button className="w-[100%] h-[100%] bg-blue-700 flex items-center justify-center"><span className="text-lg mr-2">+</span> Add Friend</Button>
                                </div>
                                <div className="w-[28%] h-[42%] m-1">
                                    <Button className="w-[100%] h-[100%] flex items-center justify-center bg-gray-300"><h1 className="text-xs text-black">Message</h1></Button>
                                </div>
                    ...
                    <div className="w-[95%] h-[30%] relative p-2 flex items-center border-t border-gray-500">
                       <div className="w-[7%] h-[100%] flex items-center justify-center hover:bg-gray-200 cursor-pointer rounded-lg" onClick={() => updateToggle(1)}> 
                            <h1 className="font-semibold text-sm">Posts</h1>
                       </div>
                       <div className="w-[7%] h-[100%] flex items-center justify-center hover:bg-gray-200 cursor-pointer rounded-lg" onClick={() => updateToggle(2)}> 
                            <h1 className="font-semibold text-sm">About</h1>
                       </div>
                       <div className="w-[7%] h-[100%] flex items-center justify-center hover:bg-gray-200 cursor-pointer rounded-lg" onClick={() => updateToggle(3)}> 
                            <h1 className="font-semibold text-sm">Friends</h1>
                       </div>
                       <div className="w-[7%] h-[100%] flex items-center justify-center hover:bg-gray-200 cursor-pointer rounded-lg" onClick={() => updateToggle(4)}> 
                            <h1 className="font-semibold text-sm">Photos</h1>
                       </div>
                       <div className="w-[7%] h-[100%] flex items-center justify-center hover:bg-gray-200 cursor-pointer rounded-lg"> 
            ...
            <div className={toggle === 1?  "show-content" : "content"}>
                ...
                <div className="w-[26%] h-[100%] flex flex-col items-center relative">
                    
                      <div className="w-[93.5%] bg-white rounded-lg mb-5 flex flex-col p-2">
                          <div className="w-[100%] h-14 flex items-center"><h1 className="font-semibold text-2xl">Intro</h1></div>
                          <div className="w-[100%] h-10 flex items-center">
                                <img className="w-[4.5%] h-[50%] mr-3" src={require("../pics/gmail.png")}/>
                                {profile.map((profile) => {
                                  return <h1 className="text-blue-500">{profile.email}</h1>
                                })}
                            </div>
                        ...
                        <div className="w-[100%] h-10 flex items-center pl-1">
                         ...
                         <div className="w-[100%] flex flex-col relative">
                             ...
                             <div className="w-[100%] flex mb-2">
                                 {userDatass.slice(0,4).map((user, index) => {
                              ...
                              <div className="w-[100%] flex mb-2">
                                      {userDatass.slice(4,8).map((user, index) => {
                            ...
                            <div  className="h-[85%] w-[100%] bg-white rounded-lg flex justify-center items-center flex-col">
                                ...
                                <div className="w-[100%] h-[50%] flex">
                                    ...
                                    <div className="w-[73%] h-[100%] flex justify-center items-center relative">
                                        ...
                                        <div className="h-[100%]">
                                            {image && <img className="rounded-xl w-[100%] h-[100%]" src={image} alt="previewImage"></img>}
                                        </div>
                                    </div>
                                ...
                                <span style={{ width: `${progressBar}%` }} className="bg-blue-700 h-[2%]"></span>
                                ...
                                <div className="w-[100%] h-[49.9%] flex justify-center items-center p-2">
                                    ...
                                    <div className="w-[30%] h-[100%] flex justify-center items-center cursor-pointer hover:bg-gray-200 rounded-lg">
                                        ...
                                        <div>
                                            ...
                                            {file && (<Button variant="text" onClick={submitImage} className="text-black p-0">Upload</Button>)}
                                        </div>
                                    </div>
            ...
            <div className={toggle === 2?  "show-about" : "content"}> 
              ...
              <div className="w-[100%] h-[100%] bg-gray-200 flex justify-center mt-1"> 
                    ...
                    <div className="w-[52%] h-[70%] bg-white ml-3 rounded-xl p-2 flex items-center flex-col justify-center">
                          <div className="w-[100%] h-[15%] mb-1 mt-1 flex">
                                ...
                                <div className="w-[83%] h-[100%] flex items-center">
                                    {profile.map((profile) =>{ return <div>
                                      <h1 className="text-m text-black">Full name: <span className="text-blue-800">{profile.name && profile.name.split(' ').slice(0).join(' ')}</span></h1>
                                    </div>})}     
                                </div>
                          ...
                          <div className="w-[100%] h-[15%] mb-1 mt-1 flex">
                                ...
                                <div className="w-[93%] h-[100%] flex items-center">
                                     {profile.map((profile) =>{ return <div>
                                          <h1 className="text-m">Nickname: <span className="text-blue-800">{profile.name && profile.name.split(' ').slice(0,1).join(' ')}</span></h1>
                                    </div>})}   
                                </div>
                          </div>
                          <div className="w-[100%] h-[15%] mb-1 mt-1 flex">
                          ...
                          <div className="w-[100%] h-[15%] mb-1 mt-1 flex">
                                ...
                                <div className="w-[83%] h-[100%] flex items-center">
                                    {profile.map((profile) =>{ return <div>
                                          <h1 className="text-m text-black">Email: <span className="text-blue-800">{profile.email}</span></h1>
                                    </div>})}    
                                </div>
                                <div className="w-[10%] h-[100%] flex items-center">
                                    <img className="w-[20%]" src={require("../pics/public.png")} alt="public"/>
                          ...
                          <div className="w-[100%] h-[15%] mb-1 mt-1 flex">
                                ...
                                <div className="w-[93%] h-[100%] flex items-center">
                                      {profile.map((profile) =>{ return <div>
                                          <h1 className="text-m text-black">Email: <span className="text-blue-800">{profile.uid}</span></h1>
                                    </div>})}  
                                </div>
                          </div>
                    </div>              
            ...
            <div className={toggle === 3?  "show-friend" : "content"}> 
                        ...
                        <div className="w-[67%] bg-white rounded-lg flex flex-col p-3 pt-5 mt-1 mb-40">                     
                          ...
                          <div className="w-[100%] flex">
                            ...
                            <div className="w-[100%] flex flex-col">
                                ...
                                <div className="w-[100%] flex">
                                      {input && filteredFriends.length === 0 ? (
                                            <div className="w-[100%] flex flex-col items-center justify-center">
                                              <div className="w-[27] h-20 flex items-center justify-center">
                                                 <h1 className="text-xl font-semibold text-gray-800">No results for "{input}"</h1>
                                              </div>  
                                            </div>
                                        ) : (
                                            filteredFriends.slice(0, 2).map((friend, index) => (
                                                <div className="rounded-lg flex items-center w-[50%] h-24 p-2 m-1 border border-gray-200" key={index}>
                                                    <div className="flex items-center justify-center cursor-pointer w-[14%] h-[100%]">
                                                        <Avatar variant="rounded" className="w-[100%] h-[100%] object-cover" src={friend.image || require('../pics/nickname.png')} alt={friend.name} />
                                                    </div>   
                                                    <Link to={`/profile/${friend.uid}`} className="flex items-center justify-start w-[65%] h-[100%]">
                                                        <div className="h-[100%] w-[100%] flex items-center pl-5">
                                                            <h1 className="font-semibold text-gray-800 text w-[100%]">{friend.name}</h1>
                                                        </div>
                                                    </Link>  
                                                    <div className="w-[22%] h-[100%] flex items-center justify-center">
                                                      <div className="w-[40%] h-[40%] bg-gray-200 flex items-center justify-center cursor-pointer rounded-lg hover:bg-gray-300"> 
                                                          <h1>...</h1>
                                                      </div>
                                                    </div>
                                                </div>
                               ...
                               <div className="w-[100%] flex">
                                      {input && filteredFriends.length === 0 ? (
                                            ...
                                            filteredFriends.slice(2, 4).map((friend, index) => (
                                                <div className="rounded-lg flex items-center w-[50%] h-24 p-2 m-1 border border-gray-200" key={index}>
                                                    <div className="flex items-center justify-center cursor-pointer w-[14%] h-[100%]">
                                                        <Avatar variant="rounded" className="w-[100%] h-[100%] object-cover" src={friend.image || require('../pics/nickname.png')} alt={friend.name} />
                                                    </div>   
               ...
               <div className="w-[67%] bg-white p-3 pt-5 flex flex-col rounded-lg mt-1">
